...
len_rows = dfc.shape[0]
#Step2，数据预处理
dfc["brand"] = dfc["brand"].apply(lambda x:x.replace("-", ""))
#拆分problem类型 => 多个字段,统计单行抱怨中所含的问题点个数
new = dfc["problem"].str.get_dummies(",")
dfc = pd.merge(dfc, new, how="outer", left_index=True, right_index=True)
dfc["prob_count"] = dfc.iloc[:, dfc.columns.get_loc("status") + 1:].sum(axis=1)
#Step3，数据统计
